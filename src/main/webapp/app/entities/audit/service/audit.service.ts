...
export class AuditService {
  ...
  protected convertDateFromClient(audit: IAudit): IAudit {
    return Object.assign({}, audit, {
      auditDate: audit.auditDate?.isValid() ? audit.auditDate.format(DATE_FORMAT) : undefined,
      createdDate: audit.createdDate?.isValid() ? audit.createdDate.format(DATE_FORMAT) : undefined,
      lastModified: audit.lastModified?.isValid() ? audit.lastModified.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((audit: IAudit) => {
        ...
        audit.createdDate = audit.createdDate ? dayjs(audit.createdDate) : undefined;
        audit.lastModified = audit.lastModified ? dayjs(audit.lastModified) : undefined;
      });
    }
    return res;
  }
