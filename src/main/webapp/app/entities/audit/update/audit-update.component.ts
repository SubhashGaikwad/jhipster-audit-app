...
export class AuditUpdateComponent implements OnInit {
  ...
  auditFormSHospGenInfosSharedCollection: IAuditFormSHospGenInfo[] = [];
  ...
  trackAuditPatientMonitoringFormById(index: number, item: IAuditPatientMonitoringForm): number {
    return item.id!;
  }

  trackAuditFormSHospGenInfoById(index: number, item: IAuditFormSHospGenInfo): number {
    return item.id!;
  }
  ...
  protected updateForm(audit: IAudit): void {
    this.editForm.patchValue({
      ...
      auditFormSHospGenInfo: audit.auditFormSHospGenInfo,
    });

    this.auditPatientMonitoringFormsCollection = this.auditPatientMonitoringFormService.addAuditPatientMonitoringFormToCollectionIfMissing(
      this.auditPatientMonitoringFormsCollection,
      audit.auditPatientMonitoringForm
    );
    this.auditFormSHospGenInfosSharedCollection = this.auditFormSHospGenInfoService.addAuditFormSHospGenInfoToCollectionIfMissing(
      this.auditFormSHospGenInfosSharedCollection,
      audit.auditFormSHospGenInfo
    );
  }
  ...
  protected loadRelationshipsOptions(): void {
    this.auditPatientMonitoringFormService
      ...
      .pipe(map((res: HttpResponse<IAuditPatientMonitoringForm[]>) => res.body ?? []))
      .pipe(
      ...
      .subscribe(
        (auditPatientMonitoringForms: IAuditPatientMonitoringForm[]) =>
          (this.auditPatientMonitoringFormsCollection = auditPatientMonitoringForms)
      );

    this.auditFormSHospGenInfoService
      .query()
      .pipe(map((res: HttpResponse<IAuditFormSHospGenInfo[]>) => res.body ?? []))
      .pipe(
        map((auditFormSHospGenInfos: IAuditFormSHospGenInfo[]) =>
          this.auditFormSHospGenInfoService.addAuditFormSHospGenInfoToCollectionIfMissing(
            auditFormSHospGenInfos,
            this.editForm.get('auditFormSHospGenInfo')!.value
          )
        )
      )
      .subscribe(
        (auditFormSHospGenInfos: IAuditFormSHospGenInfo[]) => (this.auditFormSHospGenInfosSharedCollection = auditFormSHospGenInfos)
      );
  }
  ...
  protected createFromForm(): IAudit {
    return {
      ...
      auditPatientMonitoringForm: this.editForm.get(['auditPatientMonitoringForm'])!.value,
      auditFormSHospGenInfo: this.editForm.get(['auditFormSHospGenInfo'])!.value,
    };
  }
}
