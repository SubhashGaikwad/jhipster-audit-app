...
export class AnnexureAnswersUpdateComponent implements OnInit {
  ...
  save(): void {
    ...
    if (annexureAnswers.id !== undefined) {
      this.subscribeToSaveResponse(this.annexureAnswersService.update(annexureAnswers));
    } else {
      this.subscribeToSaveResponse(this.annexureAnswersService.create(annexureAnswers));
    }
  }
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IAnnexureAnswers>>): void {
  ...
  protected updateForm(annexureAnswers: IAnnexureAnswers): void {
    ...
    this.auditsSharedCollection = this.auditService.addAuditToCollectionIfMissing(this.auditsSharedCollection, annexureAnswers.audit);
    this.annexureQuestionsSharedCollection = this.annexureQuestionsService.addAnnexureQuestionsToCollectionIfMissing(
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.annexureQuestionsService
      ...
      .subscribe((annexureQuestions: IAnnexureQuestions[]) => (this.annexureQuestionsSharedCollection = annexureQuestions));
  }
