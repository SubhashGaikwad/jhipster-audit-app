  ...
  describe('AnnexureAnswers Management Update Component', () => {
    ...
    describe('ngOnInit', () => {
      ...
      it('Should call AnnexureQuestions query and add missing value', () => {
        ...
        const annexureQuestionsCollection: IAnnexureQuestions[] = [{ id: 51516 }];
        jest.spyOn(annexureQuestionsService, 'query').mockReturnValue(of(new HttpResponse({ body: annexureQuestionsCollection })));
        const additionalAnnexureQuestions = [annexureQuestions];
        const expectedCollection: IAnnexureQuestions[] = [...additionalAnnexureQuestions, ...annexureQuestionsCollection];
        jest.spyOn(annexureQuestionsService, 'addAnnexureQuestionsToCollectionIfMissing').mockReturnValue(expectedCollection);

        activatedRoute.data = of({ annexureAnswers });
        comp.ngOnInit();

        expect(annexureQuestionsService.query).toHaveBeenCalled();
        expect(annexureQuestionsService.addAnnexureQuestionsToCollectionIfMissing).toHaveBeenCalledWith(
          annexureQuestionsCollection,
          ...additionalAnnexureQuestions
        );
        expect(comp.annexureQuestionsSharedCollection).toEqual(expectedCollection);
      });
      ...
      it('Should update editForm', () => {
        ...
        expect(comp.editForm.value).toEqual(expect.objectContaining(annexureAnswers));
        expect(comp.auditsSharedCollection).toContain(audit);
        expect(comp.annexureQuestionsSharedCollection).toContain(annexureQuestions);
      });
    });
    ...
    describe('save', () => {
      it('Should call update service on save for existing entity', () => {
        ...
        jest.spyOn(annexureAnswersService, 'update').mockReturnValue(saveSubject);
        ...
        expect(annexureAnswersService.update).toHaveBeenCalledWith(annexureAnswers);
      ...
      it('Should call create service on save for new entity', () => {
        ...
        jest.spyOn(annexureAnswersService, 'create').mockReturnValue(saveSubject);
        jest.spyOn(comp, 'previousState');
        activatedRoute.data = of({ annexureAnswers });
        comp.ngOnInit();
        ...
        // THEN
        expect(annexureAnswersService.create).toHaveBeenCalledWith(annexureAnswers);
        expect(comp.isSaving).toEqual(false);
        expect(comp.previousState).toHaveBeenCalled();
      });
